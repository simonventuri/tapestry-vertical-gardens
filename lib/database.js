...
export async function getRedisClient() {
    if (!redis) {
        ...
        redis.on('connect', () => {
            console.log('Redis connected successfully to localhost');
        });
    ...
    return withRedis(async (redis) => {
        ...
        for (const id of projectIds) {
            ...
            if (projectData.id) {
                ...
                projects.push({
                    id: projectData.id,
                    title: Array.isArray(projectData.title) ? projectData.title.join(' ') : String(projectData.title || ''),
                    ...
                    visible: projectData.visible === 'false' ? false : true, // Default to true for backwards compatibility
...
export async function getVisiblePortfolioItems() {
    ...
    return allProjects.filter(project => project.visible === true);
}
...
export async function getPortfolioItemsOptimized() {
    return withRedis(async (redis) => {
        ...
        const { generateThumbnail } = require('./imageUtils');

        // Get all project data but only include optimized thumbnail
        const projects = [];
        for (const id of projectIds) {
            const projectData = await redis.hGetAll(`portfolio:${id}`);
            if (projectData.id) {
                const images = JSON.parse(projectData.images || '[]');
                let heroThumbnail = null;

                if (images.length > 0) {
                    // Generate high-quality thumbnail for portfolio grid (600x450px max, 80% quality)
                    heroThumbnail = await generateThumbnail(images[0], 600, 450, 0.8);
                }
...
// Truly paginated function - only processes the items needed for the current page
export async function getPortfolioItemsPaginated(page = 1, limit = 4) {
    return withRedis(async (redis) => {
        // Get ordered project IDs from the sorted set (if exists), fallback to portfolio set
        let projectIds;
        const orderedExists = await redis.exists('portfolio:ordered');
        ...
        // Only get the project IDs we need for this page
        const pageProjectIds = projectIds.slice(startIndex, endIndex);

        // Import thumbnail generation function
        const { generateThumbnail } = require('./imageUtils');

        // Only process the projects we actually need
        const projects = [];
...
// Frontend version - only returns visible projects
export async function getVisiblePortfolioItemsPaginated(page = 1, limit = 4) {
    return withRedis(async (redis) => {
        // Get ordered project IDs from the sorted set (if exists), fallback to portfolio set
        let projectIds;
        const orderedExists = await redis.exists('portfolio:ordered');
        ...
        // Filter for visible projects only - only check the visible field, not full project data
        const visibleProjectIds = [];
        for (const id of projectIds) {
            const visible = await redis.hGet(`portfolio:${id}`, 'visible');
            // Include if visible field doesn't exist (backwards compatibility) or if it's not 'false'
        ...
        // Only get the project IDs we need for this page
        const pageProjectIds = visibleProjectIds.slice(startIndex, endIndex);
        ...
        for (const id of pageProjectIds) {
            ...
            if (projectData.id) {
                ...
                projects.push({
                    ...
                    slug: projectData.slug,
                    hero_image: heroThumbnail,
                    visible: projectData.visible === 'false' ? false : true,
                    createdAt: projectData.createdAt,
                    updatedAt: projectData.updatedAt
                });
            }
...
// Optimized function for admin projects list - returns minimal data for table view
export async function getPortfolioItemsForAdmin() {
    return withRedis(async (redis) => {
        // Get ordered project IDs from the sorted set (if exists), fallback to portfolio set
        let projectIds;
        const orderedExists = await redis.exists('portfolio:ordered');
        ...
        // Get minimal project data for admin table
        const projects = [];
        for (const id of projectIds) {
            const projectData = await redis.hGetAll(`portfolio:${id}`);
            if (projectData.id) {
                ...
                if (images.length > 0) {
                    // Generate small thumbnail for admin table (150x113px max, 50% quality)
                    heroThumbnail = await generateThumbnail(images[0], 150, 113, 0.5);
                }
                ...
                projects.push({
                    ...
                    hero_image: heroThumbnail,
                    images: images, // Include full images array for toggle functionality
                    imageCount: images.length, // Store actual count for display
                    visible: projectData.visible === 'false' ? false : true, // Default to true for backwards compatibility
                    createdAt: projectData.createdAt || projectData.created_at || new Date().toISOString(),
                    updatedAt: projectData.updatedAt || projectData.updated_at
                });
...
export async function createPortfolioItem(data) {
    const redis = await getRedisClient();
    const { title, description, slug, category, location, year, size, images, features, plants, visible = true } = data;
    ...
    await redis.hSet(`portfolio:${projectId}`, {
        ...
        visible: String(visible === true || visible === 'true' ? 'true' : 'false'),
...
export async function getContacts() {
    ...
    // Get all contact IDs
    const contactIds = await redis.zRange('contacts:sorted', 0, -1);
...
export async function updateProject(id, data) {
    const redis = await getRedisClient();
    const { title, description, slug, category, location, year, size, images, features, plants, visible } = data;
    ...
    const updateData = {
        ...
        visible: visible !== undefined ? String(visible === true || visible === 'true' ? 'true' : 'false') : (existingProject.visible || 'true'),
